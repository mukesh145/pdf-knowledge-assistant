from fastapi import FastAPI, HTTPException, Depends, status, Request, APIRouter, BackgroundTasks
...
try:
    from agent.orchestrator import run_ka_dag, run_ka_dag_stream, log_conversation_async
...
async def query_knowledge_assistant(
    ...
    The response is returned immediately to the user, and logging (database writes
    and S3 uploads) happens asynchronously in the background to avoid latency.
    ...
    try:
        ...
        # Run the orchestrator DAG (without logging - logging happens async)
        result = run_ka_dag(
            query=request.query,
            user_id=user_id
        )
        ...
        # Schedule logging asynchronously AFTER response is returned
        # This ensures the user gets the response immediately without waiting for logging
        background_tasks.add_task(log_conversation_async, result)
...
async def query_knowledge_assistant_stream(
    ...
    The response is streamed immediately to the user, and logging (database writes
    and S3 uploads) happens asynchronously in the background after streaming completes.
    ...
    try:
        ...
        # Use a list to store state (lists are mutable and can be modified from nested functions)
        ...
        def generate_stream():
            ...
            try:
                for chunk in run_ka_dag_stream(
                    ...
                    # Extract state from done chunk for async logging
                    if chunk.get("type") == "done":
                        done_data = chunk.get("data", {})
                        state_container[0] = done_data.get("state")
        ...
        # Schedule logging asynchronously after stream completes
        # FastAPI BackgroundTasks executes after the response is sent to the client
        # The state_container will be populated by the time the background task runs
        def log_after_stream():
            """Log conversation after stream completes."""
            import time
            # Wait briefly for state to be populated (should be immediate since stream is done)
            max_wait = 2  # Maximum wait time in seconds
            wait_time = 0
            while state_container[0] is None and wait_time < max_wait:
                time.sleep(0.1)
                wait_time += 0.1
            
            if state_container[0]:
                log_conversation_async(state_container[0])
            else:
                print("Warning: Could not retrieve state for logging after stream completed")
        
        background_tasks.add_task(log_after_stream)
